# Class to setup and manage GUI components for GFET Lab.
...
tox1 = 90#50 # TG oxide layer thickness, nm
tox2 = 1#50 # BG oxide layer thickness, nm
W = 10#40 # Channel Width, um
L = 30#120 # Channel Length, um
mup = 6500#8800 # p-type carrier mobility
mun = 4800#7700 # n-type carrier mobility
vF = 1.3*10**6 # Fermi Velocity, m/s
Nf = 5e15 # Dopant density
T = 298 # Operating temperature, default is room temp (in Kelvin)

# Default dielectric (0=SiO2, 1=SiC, 2=Al2O3, 3=AlN, 4=HfO2, 5=ZrO2, 6=hBn, 7=Air, 8=None)
topDielecDef = 0
bottomDielecDef = 8

# Default Sweep Settings
transSweepFields = 'Vtg Start', 'Vtg End', 'Vtg Step', 'Vbg', 'Vds Start', 'Vds End', 'Vds Step'
#transSweepParams = -10, 10, 0.2, 5, 0.2, 1, 0.2
transSweepParams = -36, 45, 1, -0.4, 0.01, 0.01, 0.2

ivSweepFields = 'Vtg Start', 'Vtg End', 'Vtg Step', 'Vbg', 'Vds Start', 'Vds End', 'Vds Step'
ivSweepParams = 0.2, 1, 0.2, 0, 0, 10, 0.1

fields2 = ('Top Dielectric\nThickness (nm)', 'Bottom Dielectric\nThickness (nm)', 'Channel Width (μm)', 'Channel Length (μm)', 'N-type Mobility (cm\u00b2 V⁻¹s⁻¹)',
            'P-type Mobility (cm\u00b2 V⁻¹s⁻¹)', 'Fermi Velocity (ms⁻¹)', 'Effective Dopant\nDensity (m⁻\u00b2)', 'Operating Temperature (K)')
params2 = tox1, tox2, W, L, mun, mup, vF, Nf, T
...
# Available Models
GFET_models = "Jimenez", "Mukherjee (NOT WORKING)"
...
class GUI:
    ...
    def __init__(self, master):
        ...
        self.root.title('GFET Lab')
        self.root.resizable(0,0)

        self.io = gio.GFET_IO()

        self.model = GFET_models[0] # Default to first model
...
#**************************************************************************************************#
    ...
    def genSweepModels(self, vtgModel, vdsModel, Vtg_start, Vtg_end, Vtg_step,
        ...
        if vtgModel == "Linear":
            ...
            if Vtg_start == Vtg_end:
                ...
                dps = int(VtgStepCorrection + (Vtg_end-Vtg_start)/Vtg_step)
    ...
    def loadModel(self, name, vtgModel, vdsModel, ents, ents2, ents3):
        ...
        if self.model == 'Mukherjee':
            GFET = gfet.MukherjeeGFET(params, ivSweep, transferSweep, eps)
            ...
            transferChars, gm, fT, Vtg = GFET.calculateTransferChars()
            ivChars = GFET.calculateIVChars()
            self.data["IVChars"].update({"Ids": ivChars})
            self.data["TransChars"].update({"Ids": transferChars})
            #self.data["TransChars"].update({"Vtg": Vtg})
            self.data.update({"gm": gm,
                              "fT": fT})
        elif self.model == 'Hu':
...
##        if self.modelCombo.get() == 'Mukherjee':
##            for entry in self.ents:
##                if "bg" in entry[0]:
##                    entry[1].config(state="disabled")
##        else:
...
##                entry[1].config(state="normal")
        ...
        self.modelCombo['values'] = GFET_models
    ...
    def setupParamsTab(self):
        ...
        dielecLabel1 = tk.Label(frame1, text='Top Dielectric')
        self.dielecCombo1 = ttk.Combobox(frame1, state='readonly', width=30)

        dielecLabel2 = tk.Label(frame2, text='Bottom Dielectric')
        self.dielecCombo2 = ttk.Combobox(frame2, state='readonly', width=30)

        dielecs = self.io.loadDielectrics(self.root)

        vals = []
        i = 0
        for i in range(len(dielecs)):
            vals.append(dielecs[i][0] + " (Ɛr=" + str(dielecs[i][1]) + ","  + " ħω=" + str(dielecs[i][2]) + " meV)")

        self.dielecCombo1['values'] = vals
        self.dielecCombo1.current(topDielecDef) # Fifth value in list is default

        self.dielecCombo2['values'] = vals
        self.dielecCombo2.current(bottomDielecDef) # First value in list is default
        ...
        # Only have n- or p-type mobility if Mukherjee model
        #if self.model == "Mukherjee":
        #    self.ents3 = self.makeform(self.ltab2, fields2, params2)
        if self.model == "Jimenez":
            self.ents3 = self.makeform(self.ltab2, fields2, params2)
